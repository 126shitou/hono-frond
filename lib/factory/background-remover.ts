...
const BackGroundRemoverSchema = z.object({
    images: z.array(z.any()).max(1, "You can only upload a maximum of 1 images.").optional().default([]),
});

export class BackRemoveTool implements BaseTool {
    getValidationSchema() {
        return BackGroundRemoverSchema;
    }

    async buildTaskRequest(paramaters: z.infer<typeof BackGroundRemoverSchema>) {
